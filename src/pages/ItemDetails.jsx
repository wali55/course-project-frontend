...
const ItemDetails = () => {
  ...
  const { currentItem: item, loading, error, showForm, customFields } = useSelector((state) => state.items);
    ...
    return (
      <div className="max-w-4xl mx-auto p-6">
        <div className="text-center py-12">
          ...
          <p className="text-gray-600 mb-6">The item you're looking for doesn't exist or has been deleted.</p>
          <button
            onClick={() => navigate(-1)}
          ...
          <div className="p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {Object.entries(item.fieldValues).map(([fieldName, value]) => {
                ...
                const isImage = fieldType === "IMAGE" || fieldName.toLowerCase().includes("image");
                
                return (
                  <div key={fieldName} className={isImage ? "md:col-span-2" : ""}>
                    <label className="block text-sm font-medium text-gray-500 mb-2">
                      {fieldName}
                    </label>
                    <div className="text-gray-900">
                      {renderFieldValue(fieldName, value, isImage ? "IMAGE" : fieldType)}
                    </div>
                  </div>
                );
              })}
        ...
        <div className="p-6 bg-gray-50 border-t border-gray-200">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
            ...
            <div>
              ...
              <span className="ml-2 font-mono text-gray-900">{item.id}</span>
            </div>
          </div>
        </div>
      </div>
